...
public class CoursePersistenceImpl
	...
	public List<Course> filterFindByGroupId(
		...
		if (getDB().isSupportsInlineDistinct()) {
			sb.append(_FILTER_SQL_SELECT_COURSE_WHERE);
		}
		else {
			sb.append(_FILTER_SQL_SELECT_COURSE_NO_INLINE_DISTINCT_WHERE_1);
		}

		sb.append(_FINDER_COLUMN_GROUPID_GROUPID_2);

		if (!getDB().isSupportsInlineDistinct()) {
			sb.append(_FILTER_SQL_SELECT_COURSE_NO_INLINE_DISTINCT_WHERE_2);
		}
		...
		if (orderByComparator != null) {
			...
			if (getDB().isSupportsInlineDistinct()) {
				...
				sb.append(CourseModelImpl.ORDER_BY_SQL);
			}
		}

		String sql = InlineSQLHelperUtil.replacePermissionCheck(
			sb.toString(), Course.class.getName(),
		...
		try {
			...
			if (getDB().isSupportsInlineDistinct()) {
				sqlQuery.addEntity(_FILTER_ENTITY_ALIAS, CourseImpl.class);
			}
			else {
				sqlQuery.addEntity(_FILTER_ENTITY_TABLE, CourseImpl.class);
			}
			...
			return (List<Course>)QueryUtil.list(
	...
	public int filterCountByGroupId(long groupId) {
		...
		sb.append(_FILTER_SQL_COUNT_COURSE_WHERE);
	...
	public Course findByGroupIdAndUrlTitle(long groupId, String urlTitle)
		...
		Course course = fetchByGroupIdAndUrlTitle(groupId, urlTitle);
	...
	public Course fetchByGroupIdAndUrlTitle(
		...
		if (result instanceof Course) {
			...
			if ((groupId != course.getGroupId()) ||
				!Objects.equals(urlTitle, course.getUrlTitle())) {
	...
	public Course removeByGroupIdAndUrlTitle(long groupId, String urlTitle)
		...
		Course course = findByGroupIdAndUrlTitle(groupId, urlTitle);
	...
	private static final String _FINDER_COLUMN_GROUPIDANDURLTITLE_URLTITLE_3 =
		"(course.urlTitle IS NULL OR course.urlTitle = '')";
	...
	public void cacheResult(Course course) {
		...
		dummyFinderCache.putResult(
			...
			new Object[] {course.getGroupId(), course.getUrlTitle()}, course);
	...
	protected void cacheUniqueFindersCache(CourseModelImpl courseModelImpl) {
		...
		args = new Object[] {
			courseModelImpl.getGroupId(), courseModelImpl.getUrlTitle()
		};

		dummyFinderCache.putResult(
			_finderPathCountByGroupIdAndUrlTitle, args, Long.valueOf(1));
		dummyFinderCache.putResult(
			_finderPathFetchByGroupIdAndUrlTitle, args, courseModelImpl);
	...
	private static final String _FILTER_SQL_SELECT_COURSE_WHERE =
		"SELECT DISTINCT {course.*} FROM lb_Course course WHERE ";

	private static final String
		_FILTER_SQL_SELECT_COURSE_NO_INLINE_DISTINCT_WHERE_1 =
			"SELECT {lb_Course.*} FROM (SELECT DISTINCT course.courseId FROM lb_Course course WHERE ";

	private static final String
		_FILTER_SQL_SELECT_COURSE_NO_INLINE_DISTINCT_WHERE_2 =
			") TEMP_TABLE INNER JOIN lb_Course ON TEMP_TABLE.courseId = lb_Course.courseId";

	private static final String _FILTER_SQL_COUNT_COURSE_WHERE =
		"SELECT COUNT(DISTINCT course.courseId) AS COUNT_VALUE FROM lb_Course course WHERE ";

	private static final String _FILTER_ENTITY_ALIAS = "course";

	private static final String _FILTER_ENTITY_TABLE = "lb_Course";

	private static final String _ORDER_BY_ENTITY_ALIAS = "course.";

	private static final String _ORDER_BY_ENTITY_TABLE = "lb_Course.";
